...
<div class="container">
  ...
  <section>
    ...
    {{ range .PullToJobMapping }}
      <div class="pulls-row">
      <span>{{.Pull.RepoFullName}} #{{.Pull.PullNum}}</span>
      <span><code>{{.Pull.Path}}</code></span>
      <span><code>{{.Pull.Workspace}}</code></span>
      <span>
      {{ range .JobIDInfos }}
...
<html lang="en">
  ...
  <body>
    ...
    <a title="atlantis" href="{{ .CleanedBasePath }}/"><img class="hero" src="{{ .CleanedBasePath }}/static/images/atlantis-icon_512.png"/></a>
    <p class="title-heading-white">atlantis</p>
    <p class="title-heading-white"><strong></strong></p>
    </section>
    <section>
      <div id="terminal"></div>
    </section>
